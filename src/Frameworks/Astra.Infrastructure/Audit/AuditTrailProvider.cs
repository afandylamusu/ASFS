...
{
    public class AuditTrailProvider<T> : IAuditTrailProvider<T> where T : class, IAuditTrailLog
    {
        private static string _alias = "auditlog";
           ...
           IOptions<AuditTrailOptions> auditTrailOptions)
        {
            ...
            _alias = _options.Value.Alias ?? _alias;

            _indexName = $"{_alias}-{DateTime.UtcNow.ToString("yyyy-MM-dd")}";
            ...
            var pool = new StaticConnectionPool(new List<Uri> { new Uri(_options.Value.ServiceUrl) });
        ...
        {
            Func<string> revealIndexName = () => {
                if (_options.Value.CustomIndexEnable)
                {
                    if (!string.IsNullOrEmpty(auditTrailLog.GetCustomIndexSuffix()))
                    {
                        _indexName = $"{_alias}-{auditTrailLog.GetCustomIndexSuffix().ToLower()}-{DateTime.UtcNow.ToString("yyyy-MM-dd")}";
                        if(_options.Value.IndexPerMonth)
                            _indexName = $"{_alias}-{auditTrailLog.GetCustomIndexSuffix().ToLower()}-{DateTime.UtcNow.ToString("yyyy-MM")}";
                    }
                }
                return _indexName;
            };
